...
(SELECT p.cod_plancha, p.fabricante
FROM sucursal s JOIN plancha p ON p.idsucursal = s.idsucursal;)

EXCEPT

(SELECT p.cod_plancha, p.fabricante
FROM sucursal s JOIN lavarropas l ON s.idsucursal = l.idsucursal
                JOIN plancha p ON p.idsucursal = s.idsucursal
WHERE l.origen != 'ESP';)
...
FROM sucursal s JOIN lavarropas l ON s.idsucursal = l.idsucursal
                JOIN plancha p ON s.idsucursal = p.idsucursal
                JOIN turno t ON p.codplancha = t.plancha_id
WHERE t.apodo =  'Johnny'

UNION

SELECT l.codlavarropas, l.marca
FROM lavarropas l JOIN sucursal s ON s.idsucursal = l.idsucursal
WHERE l.origen = 'BRA';


--c) Obtener la (id_sucursal, cantidad) de lavarropas por sucursal.
SELECT s.idsucursal, count(l.codlavarropas) as cantidadLavarropas
FROM sucursal s JOIN lavarropas l ON s.idsucursal = l.idsucursal
...
SELECT e.apodo, avg(t.horas) as cantHoras
FROM empleado e JOIN turno t ON e.apodo = t.apodo
                JOIN plancha p ON t.plancha_id = p.codplancha
                JOIN lavarropas l ON p.idsucursal = l.idsucursal
WHERE l.capacidad > 20
GROUP BY e.apodo
HAVING avg(t.horas) > 5;

--e) Obtener los empleados que trabajaron solamente con planchas del fabricante “Atma”.
SELECT e.apodo, t.plancha_id
FROM empleado e JOIN turno t ON e.apodo = t.apodo
